...
struct OBJECT_VOL_INFO{
  1:required i32 VolumeId     // 卷ID
  2:required i32 BeginIndex   // 卷Index
  3:required i32 BeginOffset  // 数据的起始位置
}

// 对象信息
struct OBJECT_INFOEX{
    1:required i32 ObjectSize       // 对象大小（最大4G）
    2:required i64 CreateTime       // 对象创建时间
    3:required i32 ObjectFlag       // 对象旗帜
    4:required i32 ObjectBlkSize    // 对象blk大小
    5:required string ObjectTag     // 对象标签
    6:required bool Complete        // 对象是否完成
    7:required string Hash          // 对象Hash，去重
    8:required binary CreateCtx     // 写数据ctx
}
struct OBJECT_INFO_EX_RESULT{
  1:required bfss_result.BFSS_RESULT Result=bfss_result.BFSS_RESULT.BFSS_UNKNOWN_ERROR;      // 错误代码
  2:optional OBJECT_INFOEX ObjectInfoEx                                                       // 对象信息
  3:optional OBJECT_VOL_INFO ObjectVolInfo                                                   // 对象的卷信息
}
...
service BFSS_APID{
    ...
    //-------------------------------------------------------------------------------------------------------
    // 函数名： CreateObjectEx 创建一个存储对象（SDK接口）
    // 参数：
    //  oid  对象id，最多128个英文字符（只允许'-','_','.',字母和数字），如果id已被使用这里会返回 BFSS_DUP_ID
    //  size 对象大小，这个值范围，必须在1-4G（byte）之间。
    //  flag 对象旗帜，高16位保留（必须置0），低16位调用者可自定义使用。
    //  tag  对象标签。调用者可以给对象打上标签（字符串必须是utf-8,最多128字节）。
    // 返回值：
    //      参考  OBJECT_INFO_EX_RESULT 结构体定义
    OBJECT_INFO_EX_RESULT CreateObjectEx(1:string oid, 2:i32 size, 3:i32 flag, 4:string tag=""),
    ...
    // 返回值：
    //      参考  bfss_result.BFSS_RESULT 结构体定义
    bfss_result.BFSS_RESULT ResetObjectId(1:string oid, 2:string newoid, 3:string newtag),
    ...
    //-------------------------------------------------------------------------------------------------------
    // 函数名： GetObjectInfoEx 获取对象信息（SDK接口）
    // 参数：
    //  oid 对象id，同上
    // 返回值：
    //      参考 OBJECT_INFO_EX_RESULT 结构体定义
    OBJECT_INFO_EX_RESULT GetObjectInfoEx(1:string oid),
